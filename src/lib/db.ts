 ...
 * For DDL operations (CREATE TABLE), we use the Neon NON_POOLING URL
 * because Neon's pooler endpoint (pgbouncer) doesn't support DDL.
 */
export async function ensureDbInitialized(): Promise<void> {
  ...
  // Prevent concurrent table creation
  if (ensuring) return
  ensuring = true
 ...
 * Must match the Prisma schema exactly (table names from @@map directives).
 */
 ...
 *
 * Uses the Neon NON_POOLING URL for DDL operations because
 * Neon's pooler endpoint (pgbouncer) doesn't support DDL.
 * Falls back to the default DATABASE_URL if non-pooling URL is unavailable.
 */
async function ensurePostgresTables(): Promise<void> {
  console.log('[DB] Checking PostgreSQL tables exist...')

  try {
    // Check if the main table exists using the standard connection
    ...
    // For DDL operations, we need a direct (non-pooled) connection
    // because Neon's pooler doesn't support DDL
    const directUrl =
      process.env.mojing_POSTGRES_URL_NON_POOLING ||
      process.env.mojing_DATABASE_URL_UNPOOLED ||
      process.env.DATABASE_URL

    if (directUrl && directUrl !== process.env.DATABASE_URL) {
      // Create a temporary PrismaClient with the direct URL for DDL
      console.log('[DB] Using non-pooling URL for DDL operations')
      ...
      try {
        ...
        console.log('[DB] ✅ All tables created successfully via direct connection')
      } finally {
        await directClient.$disconnect()
      }
    } else {
      // Fallback: try with the existing connection (might fail on pooler)
      console.log('[DB] WARNING: No non-pooling URL found, trying with pooler connection...')
      await db.$executeRawUnsafe(CREATE_TABLES_SQL)
      console.log('[DB] ✅ All tables created successfully via pooler connection')
    }
    ...
    if (!verify[0]?.exists) {
      console.error('[DB] ❌ Table creation appeared to succeed but tables still do not exist')
    }
  } catch (error) {
